...
def extract_bracketed_block(text, start_pattern):
...
def extract_variables(text):
    # Try to extract variables like marksPerQ, negMarks, totalDuration
    marks_match = re.search(r'(?:const|let|this\.)marksPerQ\s*=\s*([0-9.]+)', text, re.IGNORECASE)
    marks = float(marks_match.group(1)) if marks_match else 3.0
    
    neg_match = re.search(r'(?:const|let|this\.)negMarks\s*=\s*([0-9.]+)', text, re.IGNORECASE)
    neg = float(neg_match.group(1)) if neg_match else 1.0
    
    duration_match = re.search(r'(?:const|let|totalDuration|totalTimeLeft)\s*=\s*([0-9]+)', text, re.IGNORECASE)
    duration = int(duration_match.group(1)) if duration_match else 60
    
    # Try to extract the mock header/brand title
    brand_match = re.search(r'<div class="hdr-c">([^<]+)</div>', text)
    if not brand_match:
        brand_match = re.search(r'<div class="exam-title">([^<]+)</div>', text)
    brand = brand_match.group(1).strip() if brand_match else "Mocks Wallah"
    
    return marks, neg, duration, brand
...
<body>
    ...
    <div class="welcome-screen" id="welcomeScreen">
        <div class="welcome-container">
            <div class="welcome-header">
                <h1><i class="fas fa-graduation-cap"></i> CBT Portal</h1>
                <p>Computer Based Test Environment</p>
            </div>
            
            <div class="test-details">
                <div class="detail-item">
                    <div class="detail-label">Total Questions</div>
                    <div class="detail-value" id="welcomeTotalQs">0</div>
                </div>
                <div class="detail-item">
                    <div class="detail-label">Total Marks</div>
                    <div class="detail-value" id="welcomeTotalMarks">0</div>
                </div>
                <div class="detail-item">
                    <div class="detail-label">Duration</div>
                    <div class="detail-value" id="welcomeDuration">0 mins</div>
                </div>
                <div class="detail-item">
                    <div class="detail-label">Marking Scheme</div>
                    <div class="detail-value" id="welcomeMarksScheme">+0.0 / -0.0</div>
                </div>
            </div>
            
            <div class="section-info" id="welcomeSectionsBlock" style="display:none;">
                ...
                <table class="section-table">
                    ...
                    <tbody id="welcomeSectionsBody"></tbody>
                </table>
            </div>
    ...
    <div class="exam-container" id="examContainer">
        <div class="exam-header">
            <div class="exam-title">__BRAND__</div>
            <div class="exam-timer">
                <div class="timer-box" id="mainTimer">00:00</div>
                <button class="theme-toggle" id="themeToggle"><i class="fas fa-moon"></i></button>
                <button class="nav-btn btn-submit mobile-only" id="submitBtn">Submit</button>
            </div>
        </div>
        ...
        <div class="candidate-bar">
            <div class="candidate-info">
                ...
                <div class="candidate-details">
                    ...
                    <div class="candidate-id">ID: User</div>
                </div>
            </div>
            <div class="section-badge" id="sectionBadge" style="display:none;">
                <span id="sectionName"></span><span id="sectionTimer"></span>
            </div>
        </div>

        <!-- Section tabs for Soft Switching (Tier I) -->
        <div class="section-tabs" id="sectionTabs" style="display:none;"></div>
        ...
        <div class="exam-main">
            <div class="question-area">
                    ...
                    <div class="question-card">
                        <div class="question-header">
                            <div>
                                ...
                                <div class="question-marks" id="questionMarks">Marks: 3.0</div>
                            </div>
                        ...
                        <div class="comprehension" id="comprehension" style="display:none;">
                            <h4><i class="fas fa-book-open"></i> Comprehension</h4>
                            <div id="comprehensionText"></div>
                        </div>
                        ...
                        <div class="solution-box" id="solutionBox">
                            ...
                            <div class="solution-stats" id="solutionStats"></div>
                ...
                <div class="navigation-bar">
                    <button class="nav-btn btn-prev" id="prevBtn"><i class="fas fa-chevron-left"></i> Prev</button>
                    ...
                    <button class="nav-btn btn-review" id="reviewBtn"><i class="fas fa-flag"></i> Mark for Review</button>
                    <button class="nav-btn mobile-only" id="navigatorBtn"><i class="fas fa-th"></i> Grid</button>
                    <button class="nav-btn btn-submit desktop-only" id="submitBtnDesktop">Submit</button>
                    <button class="nav-btn btn-next">Next <i class="fas fa-chevron-right"></i></button>
                </div>
            </div>
            
            <div class="navigator-sidebar desktop-only">
                <div class="navigator-header">Question Navigator</div>
                <div class="navigator-grid" id="desktopNav"></div>
                <div class="navigator-legend">
                    ...
                    <div class="legend-item">
                        <div class="legend-box" style="background:var(--card);border:2px solid var(--border)"></div>
    ...
    <!-- Mobile drawer panel -->
    <div class="mobile-navigator" id="mobileNav">
        <div class="mobile-nav-header">
            <h3>Question Navigator</h3>
            <button class="close-nav" id="closeNav">&times;</button>
        </div>
        <div class="navigator-grid" id="mobileNavGrid"></div>
        <div class="navigator-legend">
            <div class="legend-item">
                <div class="legend-box" style="background:var(--success)"></div>
                <span>Answered</span>
    ...
    <div class="result-modal" id="resultModal">
        <div class="result-container">
            ...
            <div class="result-grid">
                ...
                <div class="result-stat">
                    <div class="result-stat-value" id="timeTaken">0</div>
                    <div class="result-stat-label">Minutes</div>
                </div>
            </div>
            
            <div class="section-results" id="sectionResults" style="display:none;">
                <h3>Section-wise Performance</h3>
                <table class="section-results-table">
                    <thead>
                        <tr>
                            <th>Section</th>
                            <th>Attempted</th>
                    ...
                    <tbody id="sectionResultsBody"></tbody>
                </table>
            </div>
            
            <div class="result-actions">
                <button class="result-btn btn-close" id="closeResult">Close</button>
                <button class="result-btn btn-review-ans" id="reviewAnswers">Review Answers</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        const sectionsData = __SECTIONS__;
        const marksPerQ = __MARKS__;
        const negMarks = __NEG_MARKS__;
        const totalDuration = __DURATION__;
        ...
        class CBTExamApp {
            constructor() {
                ...
                // Determine active section index for hard locks, or custom segment tabs
            ...
            setupWelcomeScreen() {
                document.getElementById('welcomeTotalQs').textContent = this.questions.length;
                document.getElementById('welcomeTotalMarks').textContent = (this.questions.length * marksPerQ).toFixed(1);
                document.getElementById('welcomeDuration').textContent = totalDuration;
                document.getElementById('welcomeMarksScheme').textContent = `+${marksPerQ.toFixed(1)} / -${negMarks.toFixed(1)}`;
            ...
            startExam() {
                ...
                // If it is Tier I (no hard sectionsData), construct segment switcher bar dynamically
                if (!this.sections || this.sections.length === 0) {
            ...
            // Dynamically construct Tier I segments (Reasoning, GK, Maths, English)
            buildSoftSections() {
                if (this.questions.length === 100) {
                    ...
                    // Fallback division by 4
                    const part = Math.floor(this.questions.length / 4);
                    this.softSections = [
                        { name: "SECTION A", start: 0, end: part - 1 },
                        { name: "SECTION B", start: part, end: (part * 2) - 1 },
                        { name: "SECTION C", start: part * 2, end: (part * 3) - 1 },
            ...
            setupEventListeners() {
                ...
                document.querySelector('.navigation-bar .btn-next').onclick = () => this.navigate(1);
                document.getElementById('reviewBtn').onclick = () => this.toggleReview();
                document.getElementById('submitBtn').onclick = () => this.confirmSubmit();
                document.getElementById('submitBtnDesktop').onclick = () => this.confirmSubmit();
                document.getElementById('navigatorBtn').onclick = () => this.openMobileNav();
                document.getElementById('closeNav').onclick = () => this.closeMobileNav();
                document.getElementById('closeResult').onclick = () => this.closeResultModal();
                document.getElementById('reviewAnswers').onclick = () => this.reviewAnswers();
            }
            ...
            updateNavigationButtons() {
                ...
                const nextBtn = document.querySelector('.navigation-bar .btn-next');
                ...
                if (this.sections && this.sections.length > 0 && !this.isSubmitted) {
                    const currentSection = this.sections[this.currentSectionIdx];
                    prevBtn.disabled = prevBtn.disabled || (this.currentQuestion === currentSection.start);
                    nextBtn.disabled = nextBtn.disabled || (this.currentQuestion === currentSection.end);
                }
            }
            ...
            showResults(results) {
                this.els.resultScore.textContent = `${results.score.toFixed(2)}/${(this.questions.length * marksPerQ).toFixed(1)}`;
...
root_dir = r"c:\Users\gagan\Downloads\Mocks\pundits"
...
def process_file(file_path):
    try:
        ...
        title = title_match.group(1).strip() if title_match else "CBT Mock Exam"
        
        # 2. Extract Questions
        questions_block = extract_bracketed_block(content, r"const\s+questions\s*=\s*\[")
        if not questions_block:
            questions_block = extract_bracketed_block(content, r"this\.qs\s*=\s*\[")
            
        if not questions_block:
            # print(f"Skipping: {file_path} - questions block not found.")
            return False
            
        # 3. Extract sectionsData
        sections_block = extract_bracketed_block(content, r"const\s+sectionsData\s*=\s*\[")
        if not sections_block:
            sections_block = "null"
            
        # 4. Extract marks, duration, and brand
        marks, neg, duration, brand = extract_variables(content)
        
        # Render template using .replace()
        rendered = TEMPLATE
        rendered = rendered.replace("__TITLE__", title)
        rendered = rendered.replace("__BRAND__", brand)
        rendered = rendered.replace("__QUESTIONS__", questions_block)
        rendered = rendered.replace("__SECTIONS__", sections_block)
        rendered = rendered.replace("__MARKS__", str(marks))
        rendered = rendered.replace("__NEG_MARKS__", str(neg))
        rendered = rendered.replace("__DURATION__", str(duration))
...
if __name__ == "__main__":
    ...
    for root, dirs, files in os.walk(root_dir):
        ...
        if ".git" in path_parts or "scratch" in path_parts:
            continue
        for f in files:
            if f.endswith(".html"):
                ...
                if success:
                    ...
                    skipped += 1
    print(f"Completed! Migrated {count} files, skipped {skipped} files.")
