...
console.log(`Patcher loaded: ${files.length} mock files to process.`);
...
files.forEach(filepath => {
  ...
  // 1. Remove Telegram Links & Text Ads
  const tgLinkRegex = /<a\s+[^>]*href=["']https?:\/\/(t\.me|telegram\.me)\/[^"']*["'][^>]*>([\s\S]*?)<\/a>/gi;
  if (tgLinkRegex.test(content)) {
    content = content.replace(tgLinkRegex, '');
    telegramFixes++;
    modified = true;
  }
  const tgTextRegex = /(join|subscribe|telegram|channel|group|pdf|updates|devgagan)[^\n]*?(t\.me|telegram\.me|@\w+)/gi;
  if (tgTextRegex.test(content)) {
    content = content.replace(tgTextRegex, '');
    telegramFixes++;
    modified = true;
  }

  // 2. Fix Absolute Paths (replace with relative hash)
  const absolutePathRegex = /(file:\/\/\/[a-zA-Z]:\/[^"'\s>]+|[a-zA-Z]:\\[^"'\s>]+)/gi;
  if (absolutePathRegex.test(content)) {
    content = content.replace(absolutePathRegex, '#');
    pathFixes++;
    modified = true;
  }

  // 3. Fix inline black text styling
  const contrastStyleRegex = /style\s*=\s*"([^"]*?)color\s*:\s*(#000|#111|black|#222|#333)(;?)([^"]*?)"/gi;
  if (contrastStyleRegex.test(content)) {
    content = content.replace(contrastStyleRegex, 'style="$1color:inherit$3$4"');
    contrastFixes++;
    modified = true;
  }

  // 4. Inject or Update Unified Dark Mode Stylesheet
  ...
  if (!styleApplied && content.includes('</head>')) {
...
console.log(`- Redesigned Style Injections: ${styleInjections}`);
console.log(`- Removed Telegram spam references: ${telegramFixes}`);
console.log(`- Corrected absolute path links: ${pathFixes}`);
console.log(`- Corrected dark text contrast rules: ${contrastFixes}`);
